...
class Family:
    def __init__(self, asset, order, separation, children, adults, rich_flag, flag):
        self.asset = asset
        self.order = order
...
def distribution(asset, order, population):
    dist = np.exp(- order * np.arange(population))
    dist = asset * dist / np.sum(dist) - epsilon
...
def generation(families):
    ...
    for family in families:
        family.asset = (1 - decay) * family.asset
        if random.random() > family.separation:
            dist = distribution(family.asset, family.order, family.children)
            for asset in dist:
                if asset > 0:
                    if asset == dist[0]:
                        flag = 1
                        ...
                        flag = 0
                    next_gen.append(Family(asset, family.order, family.separation, 0, 1, family.rich_flag, flag))
    ...
    for family in families:
        family.asset +=  rate * (1 + max(-0.9, random.gauss(0.0, noise))) * (1 + family.asset) * math.log(family.adults + 1)

        if family.children > 0:
            dist = distribution(family.asset, family.order, family.adults)
            for asset in dist:
                if asset > 0:
                    if asset == dist[0]:
                        flag = 1
                        ...
                        flag = 0
                    children = np.random.poisson(birth + asset * feedback)
                    if children > 0:
                        ...
                        next_gen.append(Family(asset, order, separation, children, 0, family.rich_flag, flag))
        else:
            family.children = np.random.poisson(birth + family.asset * feedback)
    ...
    if len(families) > 0:
        cur = np.array([family.asset for family in families])
    ...
    return families, population, orders, separations, [family.asset for family in families if family.flag == 1], [family.asset for family in families if family.flag == 0], np.mean(np.array([family.rich_flag for family in families if family.flag == 1]))
...
def main():
    ...
    if len(communities) > 0:
        df_res = pd.DataFrame(index = ["birth", "mutation", "epsilon",  "num_community", "num_family", "cap", "feedback", "decay", "noise", "asset", "order", "separation",  "rich_flag", "half_index", "rich_exp", "poor_power","gibrat"])
        for community in communities:
            cur = np.array([[family.asset, family.order, family.separation, family.rich_flag] for family in community.families])
            assets, orders, separations, rich_flags = [], [], [], []
            for i in range(iteration - 100, iteration):
                assets.extend(community.df.iat[2,i])
            ...
            try:
                ...
                res = [np.mean(np.array(assets)), np.mean(np.array(orders)), np.mean(np.array(separations)), rich_flag, half_index, rich_exp, poor_power, gibrat]
...
for epsilon in [0.00001, 0.00003, 0.0001, 0.01, 0.0003, 0.03, 0.001, 0.003]:
    for cap in [3, 0.7, 1.5, 1.2, 1, 0.8, 2]:
        df_res = pd.DataFrame()
